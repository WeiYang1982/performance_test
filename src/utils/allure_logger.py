...
class AllureLogger(logging.Logger):
    ...
    def _log(self, level, msg, args, exc_info=None, extra=None, stack_info=False, stacklevel=1, driver=False):
        """
        ...
        if logging._srcfile:
            ...
            fn, lno, func = "(unknown file)", 0, "(unknown function)"
        record = self.makeRecord(self.name, level, fn, lno, msg, args, None, func, None, None)
        ...
        with allure.step(title):  # 输出step
            if driver:  # 如果参数中有webDriver对象则进行截屏
                ...
                ScreenShot.take_screenshot(driver, "快照")
                pass
            if exc_info:
